...
OUT=/tmp/ds-attempt-output.$$
ARMED=/etc/arxos/deathstroke/ARMED
DISPOSABLE=/etc/arxos/deathstroke/DISPOSABLE_MACHINE_OK_TO_DESTROY
made_armed=0
made_disposable=0
...
if [ ! -e "$DISPOSABLE" ]; then touch "$DISPOSABLE"; made_disposable=1; fi
if [ ! -e "$ARMED" ]; then touch "$ARMED"; made_armed=1; fi
...
run(){ env DS_TEST_MODE=1 DS_STATE_DIR="$DS_STATE_DIR" DS_LOCKOUT_AT=3 DS_WIPE_AT=5 DS_COOLDOWN=300 DS_LOCKOUT_DELAY=0 \
        DS_FIRE_CMD="touch $FIRED" "$HARNESS" "$SERVICE" tester "$1" >"$OUT" 2>&1; echo $?; }
cnt(){ cat "$DS_STATE_DIR/attempts" 2>/dev/null | awk '{print $1}'; }
fired(){ [ -f "$FIRED" ] && echo y || echo n; }
cleanup(){
  rm -f "$SVC" "$CHECK" "$FIRED" "$OUT"
  [ "$made_armed" = 1 ] && rm -f "$ARMED"
  [ "$made_disposable" = 1 ] && rm -f "$DISPOSABLE"
  return 0
}
trap cleanup EXIT
...
echo "== corrupt armed verifier FAILS CLOSED (deny, never fall through) =="
printf 'not-a-valid-verifier\n' > "$DS_STATE_DIR/duress.hash"
rm -f "$FIRED"; rc=$(run "$REALPW")
{ [ "$rc" != 0 ] && [ "$(fired)" = n ] && grep -q 'armed verifier invalid' "$OUT"; } \
  && P "corrupt verifier denies authentication without firing" || F "corrupt verifier failed open (rc=$rc)"

echo "== corrupt armed counter FAILS CLOSED on an auth failure =="
python3 - "$DURESS" "$DS_STATE_DIR/duress.hash" <<'PY'
import sys,os,hashlib,binascii
code,out=sys.argv[1].encode(),sys.argv[2]
it=200000; salt=os.urandom(16)
open(out,'w').write(f"pbkdf2_sha256${it}${binascii.hexlify(salt).decode()}${binascii.hexlify(hashlib.pbkdf2_hmac('sha256',code,salt,it,32)).decode()}")
PY
printf 'garbage\n' > "$DS_STATE_DIR/attempts"
rm -f "$FIRED"; rc=$(run wrong-after-corruption)
{ [ "$rc" != 0 ] && [ "$(fired)" = n ] && grep -q 'cannot durably record failure' "$OUT"; } \
  && P "corrupt counter denies authentication instead of resetting to zero" || F "corrupt counter weakened policy (rc=$rc)"
